...
- Calculation fixes keep map-matching confidence and snapped coverage numeric, omit invalid speed limits from popups, preserve Android `ON_BICYCLE` as `on_bicycle` while retaining legacy `cycling`, and make native platform checks module-level constants.
...
- Android native tracking constants now name the 120-second stats gap, 2-minute Usage Access lookback, sustained-turn heading threshold, and TTS speech rate, and the stats loop uses a single explicit positive-duration guard.
