...
function buildDoc() {
  ...
  doc.push(table(
    ...
    [
      ...
      ['Stop-start pattern proxy (legacy following fields)', '`detectTailgateCycles` detects repeated cruise/deceleration patterns from GPS speed only: at least 4 seconds at 40 km/h or above, then braking and a speed drop. It cannot measure distance to a lead vehicle. The compatibility field `following_distance_score` is null below 0.5 km and otherwise contributes 10% of Safety.'],
      ['Braking-onset timing proxy (legacy reaction fields)', '`calculateReactionTimeProxy` times a GPS speed-drop precursor against a harsh-brake event and reports only after three usable events. It cannot observe a hazard stimulus or a driver response, so `reaction_score`, `avg_reaction_seconds`, and the current CSV/UI reaction wording are not human reaction-time measurements. The value currently contributes 10% of Smoothness.'],
      ['Estimated close-proximity alert', 'New detections are `close_proximity` events when braking and heading change coincide above 40 km/h, with defaults of 3.5 m/s2 braking and 30 deg/s heading rate. With no forward-facing sensor this does not establish a near miss or proximity to another object. Direct Safety/Smoothness event penalties are zero; legacy `near_miss_*` aliases still feed predictive-route and weather risk calculations.'],
      ['GPS heading-drift drowsy proxy', '`detectDrowsyDriving` evaluates 45-60 second all-above-80 km/h GPS-heading windows when advanced safety detection is enabled. Curvature and GPS noise can resemble drift, and no driver-monitoring evidence is available; `drowsy_risk_*` must not be read as a drowsiness diagnosis.'],
      ['Heading-deviation event (legacy lane-change fields)', '`detectLaneChanges` looks for counter-steering GPS-heading shapes above 50 km/h and emits events labelled `possible lane change` with medium confidence after contextual suppression. It has no lane geometry or camera confirmation, although current `lane_change` fields and Safety penalties remain in the compatibility/scoring surface.'],
      ...
      ['Predictive route normalization', '`estimatePredictiveRouteRisk` normalizes and clamps baseline, event-density, danger-zone, weather, and time inputs before applying fractional weights. The current density path still counts legacy `near_miss_count` / estimated close-proximity aliases at double event weight. Route-risk indexing snaps nearby GPS cells within 15 m to limit same-road fragmentation.'],
  ...
  doc.push('');
  doc.push('> GPS-ONLY SAFETY LIMITATION: Road Sage has no hazard-stimulus timestamp, lead-vehicle ranging sensor, lane camera/HD-lane geometry, or driver-monitoring sensor. Braking-onset, stop-start, close-proximity, heading-deviation, and drowsy-heading outputs are low-confidence behavior proxies, regardless of legacy UI, CSV, or storage field names.');
  doc.push('');
