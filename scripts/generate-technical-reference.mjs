...
function envTable() {
  ...
  const envDefaults = {
    ...
    VITE_ALLOW_UNVERIFIED_PLAY_INTEGRITY: 'false',
  };
  const envDescriptions = {
    ...
    VITE_ALLOW_UNVERIFIED_PLAY_INTEGRITY: 'Development/test-only override that permits raw Play Integrity tokens without a backend-verified verdict; production native sensitive actions should leave this unset.',
...
function buildDoc() {
  ...
  doc.push('- Runtime integrity: `RuntimeIntegrityCheck` detects local root, debugger, emulator, and ADB-enabled signals. `MainActivity` suspends active tracking when a compromised status is reported, and the `PlayIntegrity` bridge requests Play Integrity API tokens with per-action nonces so native calibration uploads and encrypted exports can be gated before sensitive data leaves the app. Calibration uploads assert backend verification flags from the server response; local encrypted exports require a native Play Integrity token before writing the `.rsexport` file but do not require backend verification because no export service receives the data. Strict raw-token-only acceptance for backend-bound actions remains limited to explicit development/test override via `VITE_ALLOW_UNVERIFIED_PLAY_INTEGRITY=true`.');
...
function buildReadme() {
  ...
  return [
    ...
    '- `RuntimeIntegrityCheck` flags local root, debugger, emulator, and ADB-enabled signals; compromised status suspends active tracking and the `PlayIntegrity` bridge requests nonce-bound Play Integrity tokens for sensitive native actions. Calibration uploads require backend-verified Play Integrity verdicts, while local encrypted exports require a native token before writing the password-protected wrapper without sending export data to a verification service.',
