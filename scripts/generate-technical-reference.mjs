...
function buildReadme() {
  ...
  return [
    ...
    '- Android release builds disable app backup, exclude sensitive storage from backup/data extraction, deny cleartext traffic through network security config, trust only system public CAs, run R8/resource shrinking and obfuscation, strip `android.util.Log` calls with R8, mark the main window `FLAG_SECURE`, request StrongBox-backed Android Keystore keys when available, keep Capacitor reflection entry points and native JSON model members in ProGuard, disable Capacitor bridge logging and `CapacitorHttp`, sync only allowlisted plugins, and use explicit immutable PendingIntent flags for app-owned intents.',
    ...
    '- `RuntimeIntegrityCheck` flags local root, debugger, emulator, and ADB-enabled signals; compromised status suspends active tracking and is exposed through the `PlayIntegrity` bridge. Full Play Integrity API server nonce verification is not implemented in this repo.',
