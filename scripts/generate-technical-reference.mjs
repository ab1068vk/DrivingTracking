...
function buildDoc() {
  ...
  doc.push(table(
    ...
    [
      ...
      ['Route risk segment weighting', '`buildRouteRiskIndex` applies named internal pattern weights through `ROUTE_RISK_CONSTANTS`: a general observed event adds `ROUTE_RISK_EVENT_WEIGHT = 20` points per traversal rate and a harsh-event classification adds `ROUTE_RISK_HARSH_WEIGHT = 40` additional points per traversal rate. An event is attributed to a segment only within `ROUTE_RISK_EVENT_SNAP_DISTANCE_M = 120` of its midpoint, and events whose coordinates were removed by privacy masking are rejected rather than read as (0, 0). These map and alert indicators are not calibrated to collision or casualty outcomes.'],
      ['Repeated event area policy', 'A repeated event area requires repetition across drives, not a raw event count. `buildDangerZones` clusters `harsh_brake`, `sharp_turn`, and `rapid_acceleration` events by true distance within `DANGER_ZONE_CLUSTER_RADIUS_M = 60` using union-find rather than a rounded lat/lng grid, so no rounding boundary can split a genuine cluster, and admits an area only at `DANGER_ZONE_MIN_TRIPS = 2` or more distinct trips alongside the event floor (`DANGER_ZONE_MIN_EVENTS = 3` for display, `DANGER_ZONE_ALERT_MIN_EVENTS = 2` for the stored set the live warning reads). Where a route-risk index is supplied, areas carry `passes` and `eventRate` and are ranked by rate, so a place that catches the driver on most passes outranks a road merely driven often; without an index both are null rather than zero. Speeding is excluded from point clustering by design, because one continuous over-limit run emits a single event positioned at whichever fix was fastest and so does not repeat in one place; habitual speeding is derived instead by `buildSpeedingStretches` from route-risk segments, requiring `SPEEDING_STRETCH_MIN_PASSES = 4` passes and an over-limit share of `SPEEDING_STRETCH_MIN_RATE = 0.5`. Zone identity is derived from the cluster centre, so the first drive after upgrading may re-alert for an area alerted under the previous grid-derived id. Stretches are shown alongside areas but are not written to the stored zone set that drives native proximity alerts. All thresholds live in `src/lib/appConstants.js` and are provisional pattern thresholds, not calibrated to collision or casualty outcomes.'],
      ['Predictive hazard horizon policy', 'Live hazard warnings are ranked by time-to-hazard along a projected forward corridor rather than by radial distance. `src/lib/hazard/*` resolves a travel heading from the GPS bearing above `HAZARD_HEADING_MIN_TRUST_SPEED_KMH` or derives one over a `HAZARD_HEADING_BASELINE_M = 25` displacement baseline, and abstains entirely when neither is available. Hazards behind the vehicle, outside the widening corridor, beyond `HAZARD_HORIZON_ALERT_SECONDS`, or already inside `HAZARD_HORIZON_MIN_SECONDS` are suppressed with a named reason. `hazardAlertGate` additionally requires closure consistent with the vehicle\'s own speed across consecutive fixes, so a hazard on a parallel or diverging road does not accumulate, and alerts once per hazard per drive under a global cooldown and per-drive ceiling. The `late_braking_pattern` advisory is drawn only from the driver\'s own route-risk segments and fires only when the approach exceeds their own recorded typical entry speed; it is named for the repeated hard-braking it observes rather than asserting curvature. Hazard knowledge is privacy-zone filtered when the snapshot is built. All `HAZARD_*` values live in `src/lib/appConstants.js`, are mirrored into Android `DetectionConstants.java`, and are provisional ergonomics assumptions that are not calibrated to collision or casualty outcomes.'],
  ...
  doc.push('');
  doc.push('Entry points: `index.html` loads `src/main.jsx`; `src/App.jsx` defines app routes and bootstraps theme, notifications, onboarding, app lock, screen-capture policy, privacy-zone loading, key rotation, raw GPS retention, road-context/rescore workers, and Android auto tracking; `android/app/src/main/java/com/drivesense/app/MainActivity.java` hosts the Capacitor app; `android/app/src/main/java/com/drivesense/app/DriveSenseAutoTrackingService.java` handles native tracking.');
  doc.push('');
  doc.push('---');
