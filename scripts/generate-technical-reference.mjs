...
function buildDoc() {
  ...
  doc.push('');
  doc.push('Entry points: `index.html` loads `src/main.jsx`; `src/App.jsx` defines app routes and bootstraps theme, notifications, onboarding, and Android auto tracking; `android/app/src/main/java/com/roadsage/app/MainActivity.java` hosts the Capacitor app; `android/app/src/main/java/com/roadsage/app/RoadSageAutoTrackingService.java` handles native tracking.');
  doc.push('');
  doc.push('---');
  ...
  doc.push('');
  doc.push('Core persisted models are plain JSON trip, vehicle, settings, backup, diagnostic, route-risk-index, privacy-zone, motion-sample, OBD-annotated route-point, and Android native event records. Completed trips now include canonical `component_scores` evidence envelopes, `score_provenance`, and `score_explanation` for scoring version/input auditing and plain-language score contributors. `src/lib/schema/tripSchema.js` documents the canonical v23 trip shape and required fields. Local trip reads run a one-time retired-event migration that converts legacy `lane_change` records to diagnostic `heading_deviation_legacy` records and remaps matching event-feedback keys. There is no ORM schema in this repo; IndexedDB schema creation lives in `src/lib/localTripRepository.js`, local/mobile storage helpers live in `src/lib/mobileStorage.js` and `src/lib/trackingStore.js`, and Android SharedPreferences records live in `DriveSenseNativeTripStore.java` and `RoadSageAutoTrackingService.java`.');
  doc.push('');
...
function buildReadme() {
  ...
  return [
    ...
    '- Map surfaces with route simplification, stop handling, privacy-masked coordinate handling, HTML-escaped Leaflet popups, speed-limit coloring, fatigue overlays, event markers, repeated-route comparison support, last-parked and last-known context, and TripMap no-location empty states instead of hardcoded city defaults.',
