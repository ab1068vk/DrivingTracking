...
function buildDoc() {
  ...
  doc.push(table(
    ...
    [
      ...
      ['Scoring calibration and provenance', '`src/lib/scoringConstants.js` owns provisional scoring/risk/UBI constants and their affected metrics. Stored score output is versioned as `SCORING_VERSION = 2.2.0`, and each newly scored trip stores `score_provenance` with its component evidence and scoring-input snapshot. Settings lists provenance/input mismatches for explicit re-score actions and marks provisional output as approximate. `PENALTY_SCALE_FACTOR_CALIBRATION_PROCESS` documents the labeled-dataset requirements, fitting command, validation outputs, and promotion checklist before penalty-rate scaling can be treated as calibrated.'],
...
function buildReadme() {
  ...
  return [
    ...
    '- Trip scoring for safety, GPS motion smoothness, eco driving estimates, confirmed Android Usage Access phone-use evidence, speed compliance with posted-or-inferred limit provenance, road-type segments, brake onset smoothness, cornering, braking efficiency, contextual urban/highway stop-start patterns, fatigue exposure, heading drift Beta, source-attributed weather context, estimated brake-turn manoeuvre alerts, historical context signals, and diagnostic-only GPS phone/overtake pattern counts.',
