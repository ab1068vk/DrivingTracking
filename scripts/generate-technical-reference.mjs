...
function buildDoc() {
  ...
  doc.push(table(
    ...
    [
      ...
      ['Primary storage', 'IndexedDB/localStorage/Capacitor Preferences for web/non-native UI state, encrypted Android SharedPreferences for native tracking, native settings, native notification state, privacy zones, and parked-location data, Android widget cache files, native motion samples, and native download files. Android production paths no longer read legacy plaintext `SharedPreferences`/Capacitor Preferences for native settings, privacy zones, tracking, notifications, or parked locations; legacy native plaintext preference files are delete-only cleanup targets.'],
      ...
      ['Android parked-car widget', '`ParkedCarWidgetProvider` reads the last parked location from encrypted native trip storage, shows a dashboard deep link, provides a geo navigation intent, and schedules `MapTileFetchWorker` to cache a pinned static map image per widget. `MapTileFetchWorker` checks privacy zones before any static-map or Nominatim request; private parked locations show the local privacy placeholder and do not send coordinates to external map/geocoding services.'],
...
function buildReadme() {
  ...
  return [
    ...
    '## Current App Surface',
    '',
    '- Dashboard, trip history, trip detail, live map, driving coach, insights, achievements, reports, diagnostics, searchable modular settings, and vehicles pages.',
    ...
    '- Privacy-zone and map fixes keep private locations masked, block Android widget static-map and reverse-geocoding requests for private parked coordinates, allow radius editing, hide private events, exclude masked null coordinates from distance/playback math, HTML-escape user/external values in Leaflet popups, and preserve original GPS geometry when route snapping or old map-matching data would collapse playback.',
    ...
    '- Primary storage: IndexedDB, localStorage, Capacitor Preferences for web/non-native UI state, encrypted Android SharedPreferences for native tracking, native settings, native notification state, privacy zones, and parked-location data, native motion samples on trips, native download files, and Android widget map cache files. Android production paths no longer read legacy plaintext `SharedPreferences`/Capacitor Preferences for native settings, privacy zones, tracking, notifications, or parked locations; legacy native plaintext preference files are delete-only cleanup targets.',
