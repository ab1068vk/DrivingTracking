...
function buildReadme() {
  ...
  return [
    ...
    '- Backup import is hardened and versioned: v1-v8 backups migrate before merge, retired `lane_change` events become diagnostic `heading_deviation_legacy` events, local calibration survey labels are preserved in full backups, deterministic privacy-zone cell hashes are omitted from exports, files over 50 MB are rejected before reading, records are sanitized, trip notes allow 10,000 characters, and any truncation reports the affected trip count and requires explicit user acknowledgement before import completes.',
