...
function topCalculationSnippets() {
  const snippets = [
    ...
    functionSnippet('Historical context risk', 'src/lib/predictiveRouteRisk.js', 'estimatePredictiveRouteRisk'),
...
function buildDoc() {
  ...
  doc.push(table(
    ...
    [
      ...
      ['Historical context normalization', '`estimatePredictiveRouteRisk` normalizes and clamps scored personal baseline, verified driving-event density, repeated driving-event areas, weather, and time inputs before applying fractional weights. `ROUTE_RISK_CONSTANTS.EVENT_DENSITY_MAX_EVENTS_PER_KM = 5` and `DANGER_ZONE_SATURATION_COUNT = 5` are named provisional saturation assumptions, not collision- or casualty-calibrated thresholds. Low-confidence proxy counts such as current `close_proximity_count` and legacy `near_miss_count` are excluded from context-risk weighting. The dashboard labels this as an estimated historical-context signal and exposes weighted signal contributions. Repeated-event route indexing snaps nearby GPS cells within 15 m to limit same-road fragmentation and excludes proxy events.'],
      ...
      ['Legacy and native score provenance', 'Legacy completed trips without current provenance are tagged `unknown_legacy_unrescored` rather than being marked as current scoring output. Android native completed trips write null score fields with `score_status: pending_javascript_scoring` until JavaScript scoring calculates evidence-backed values.'],
...
function buildReadme() {
  ...
  return [
    ...
    '- Trip scoring for safety, GPS motion smoothness, eco driving estimates, confirmed Android Usage Access phone-use evidence, speed compliance, road-type segments, brake onset smoothness, cornering, braking efficiency, stop-start patterns, fatigue exposure, heading drift Beta, slippery-condition, estimated brake-turn manoeuvre alerts, historical context signals, and diagnostic-only GPS phone/overtake pattern counts.',
    ...
    '- Historical context risk replaces route-prediction wording unless a real planned route is supplied. It requires observed completed-trip distance and a scored-distance baseline; without either, the dashboard shows Not enough driving history / Not enough scored driving history and no risk number. Once evidence exists, it sorts completed trips newest-first, excludes unscored trips and tiny-trip event-density distortion, normalizes all weighted components to 0-100, and clamps weather/baseline input before scoring. Its named saturation assumptions remain provisional: five eligible verified driving events per km or five nearby repeated driving-event areas each saturate their respective signal without a collision-outcome calibration claim. Low-confidence proxy events such as current brake-turn alerts and ambiguous legacy `near_miss` counts are excluded from context-risk weighting. The dashboard labels the value as estimated and exposes weighted signal contributions. Repeated-event route index cells are coarser, merge nearby segment midpoints within 15 m to reduce GPS fragmentation, and exclude proxy events.',
    ...
    '- Trip Detail and Map no longer silently hide additional repeated-event route stretches or repeated driving-event areas: initial lists remain compact, and show-all controls report hidden counts. Heading drift Beta color and fatigue critical markers now follow actual levels and exported thresholds, and compliance bars use the canonical score color tiers.',
