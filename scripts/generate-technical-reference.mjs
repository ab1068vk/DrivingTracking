...
function buildDoc() {
  ...
  doc.push(table(
    ...
    [
      ...
      ['Metric and confidence contract', '`src/lib/metricRegistry.js` defines display names, descriptions, sources, evidence minimums, and calibration notes for component/report metrics. `component_scores` envelopes carry value, internal evidence (`high`, `developing`, `low`, or `unavailable`), data sources, and sample counts; public score surfaces render `developing` as "limited evidence", suppress `high` evidence badges, and suppress unavailable values rather than rendering zero. CSV exports add a metric-metadata row and monthly/UBI PDFs add metric-reference pages.'],
      ['Scoring calibration and provenance', '`src/lib/scoringConstants.js` owns provisional scoring/risk/UBI constants and their affected metrics. Stored score output is versioned as `SCORING_VERSION = 2.2.0`, and each newly scored trip stores `score_provenance` with its component evidence and scoring-input snapshot. Settings lists provenance/input mismatches for explicit re-score actions and marks provisional output as approximate.'],
      ...
      ['Fatigue-to-Safety deduction policy', '`FATIGUE_SAFETY_PENALTY_SCALE = 0.15` is a cited conversion from the normalized 0-100 fatigue proxy into raw Safety penalty points. Maximum reported fatigue therefore adds 15 raw penalty points before trip-distance normalization, mapping conservatively to the 0.05% BAC-equivalent impairment level reported by Williamson & Feyer (Occupational and Environmental Medicine, 2000); this coefficient is not calibrated against collision outcome data.'],
      ...
      ['Weather context availability', '`fetchWeatherContext` returns `riskLevel: null` and `riskScore: null` when weather is disabled, the route is empty, or Open-Meteo has no matching hourly sample. `applyWeatherRiskToScores` preserves unavailable weather as null, and historical-context weighting displays weather as Unavailable with neutral contribution instead of pretending missing weather is low risk.'],
      ...
      ['Trip readiness display honesty', '`computePreTripRisk` can still return internal readiness evidence as `developing`, but Dashboard now displays limited-data readiness copy unless evidence is high and withholds a confident readiness number when the input signal set is too thin.'],
...
function buildReadme() {
  ...
  return [
    ...
    '- Diagnostics capture unhandled app errors, handled critical operation failures, and isolated React section crashes with sanitized messages and stack previews; development builds can seed and remove local synthetic trips only behind the explicit synthetic-test-data guard.',
    '',
    ...
    '- Open road context is explicit and privacy-aware. OpenStreetMap speed limits and Open-Meteo weather are manual by default unless automatic context fetch is enabled. When weather is disabled, unavailable, or has no matching hourly sample, the app stores weather risk as null and displays it as unavailable instead of defaulting to low risk. When a posted map speed limit is absent, Settings can choose Global, Canada, United States, or United Kingdom approximate road-type defaults, with the chosen fallback provenance preserved in reports. OSRM route snapping is opt-in, disabled without a configured endpoint, the public demo requires confirmation, and the custom endpoint field warns that raw sampled GPS coordinate pairs are sent to the configured OSRM server.',
    ...
    '- Score rings now use the canonical `getScoreColor()` metadata, including SVG stroke colors, so score labels, fills, and circular rings share one color policy. They render developing internal evidence as "limited evidence" and omit high-evidence labels to reduce repetitive confidence copy.',
