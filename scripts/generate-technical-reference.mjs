...
function buildDoc() {
  ...
  doc.push('- Dependency monitoring: security CI runs production `npm audit --audit-level=high --omit=dev`, all-dependency critical audit, lint, tests, Android instrumentation, build checks, and production-bundle debug-route scans. Dependabot is configured for npm and Gradle update PRs, with security-sensitive package grouping.');
...
function buildReadme() {
  ...
  return [
    ...
    '- Manual trip capture, foreground auto-detect, and Android native background auto tracking with activity recognition, GPS fallback, quick settings tile support, pause/resume controls, permission-health warnings, native trip import, and Android parked-car widget support.',
    ...
    '- Full backup export/import for trips, GPS route points, events, vehicles, settings, privacy-zone metadata, saved filters, reviewed event feedback, score explanations, lane-changing evidence, fallback speed-limit provenance, and legacy heading-event migration. Exports default to password-protected AES-GCM `.rsbackup` files; plaintext opt-out backups are HMAC-sealed, and imports require confirmation before truncating notes.',
    ...
    '- The Android parked-car widget reads the last parked location from encrypted native storage, opens Road Sage on widget tap, launches geo navigation from its Navigate button, and uses `MapTileFetchWorker` to cache a pinned static map image per widget. Privacy zones are checked before cached tiles are served or static-map/Nominatim requests are made; private parked locations delete stale coordinate-keyed cache files and show a local placeholder instead of sending coordinates externally.',
    ...
    '- CI runs npm audit gates for high/critical production CVEs and critical all-dependency CVEs, stable unit/component tests, Playwright browser smoke, Android emulator instrumentation, production build checks, and debug-route bundle scans on pushes and pull requests. Dependabot opens weekly npm and Gradle update PRs, while live Overpass, Open-Meteo, and OSRM checks remain manual or weekly because they depend on public external services.',
    ...
    '- Native stack: Capacitor 8 Android shell plus custom Java services/plugins for activity recognition, background tracking, phone usage evidence, native IMU motion sampling, native downloads, notifications, quick settings tile, parked-car widget, WorkManager static-map cache, encrypted SharedPreferences storage, and Android security hardening',
    ...
    '- Optional external services: OpenStreetMap Overpass for speed limits, Open-Meteo for weather context, Nominatim reverse geocoding outside privacy zones, trusted user-configured OSRM for route snapping after explicit consent, and staticmap.openstreetmap.de for non-private Android parked-car widget map previews. Set `VITE_OSRM_TIMEOUT_MS` to tune the build default OSRM timeout; users can override it in Settings from 5 to 30 seconds.',
    '',
    ...
    '- The generated CSP allows app code from `self`, disallows inline scripts, restricts network connections to `self`, Overpass, Open-Meteo, Nominatim, staticmap.openstreetmap.de, and `VITE_API_URL` when configured, and reports violations to `/csp-report` when served as an HTTP header. Android also injects CSP and security headers into Capacitor WebView local responses.',
    '- Android release builds disable app backup, exclude sensitive storage from backup/data extraction, deny cleartext traffic through network security config, trust only system public CAs, run R8/resource shrinking and obfuscation, strip `android.util.Log` calls with R8, mark the main window `FLAG_SECURE`, keep Capacitor reflection entry points and native JSON model members in ProGuard, and use explicit immutable PendingIntent flags for app-owned intents.',
