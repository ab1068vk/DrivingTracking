...
function buildDoc() {
  ...
  doc.push(table(
    ...
    [
      ...
      ['Primary storage', 'IndexedDB/localStorage/Capacitor Preferences/Android SharedPreferences, Android widget cache files, native motion samples, and native download files.'],
      ...
      ['Android parked-car widget', '`ParkedCarWidgetProvider` reads the last parked location from native trip storage, shows a dashboard deep link, provides a geo navigation intent, and schedules `MapTileFetchWorker` to cache a pinned static map image per widget. The widget map fetch sends the saved parked coordinate to `staticmap.openstreetmap.de` only when a widget with a saved location needs a refreshed image.'],
  ...
  doc.push('');
  doc.push('Core persisted models are plain JSON trip, vehicle, settings, backup, diagnostic, route-risk-index, privacy-zone, motion-sample, OBD-annotated route-point, Android native event records, saved parked-location records, and widget map cache files. Completed trips now include canonical `component_scores` evidence envelopes, `score_provenance`, and `score_explanation` for scoring version/input auditing and plain-language score contributors. `src/lib/schema/tripSchema.js` documents the canonical v23 trip shape and required fields. Local trip reads run a one-time retired-event migration that converts legacy `lane_change` records to diagnostic `heading_deviation_legacy` records and remaps matching event-feedback keys. There is no ORM schema in this repo; IndexedDB schema creation lives in `src/lib/localTripRepository.js`, local/mobile storage helpers live in `src/lib/mobileStorage.js` and `src/lib/trackingStore.js`, Android SharedPreferences records live in `DriveSenseNativeTripStore.java` and `RoadSageAutoTrackingService.java`, and parked-widget image cache files are stored under Android app files by `MapTileFetchWorker.java`.');
  doc.push('');
  ...
  doc.push('- External data sharing: Overpass gets route-area boxes, Open-Meteo gets midpoint/date, OSRM receives sampled raw GPS coordinate pairs only when route snapping is explicitly enabled, a trusted endpoint is saved, data sharing has been consented to, and the user requests road data, and the Android parked-car widget can send the saved parked coordinate to `staticmap.openstreetmap.de` when it refreshes its static map image. Settings rejects saving the public OSRM demo endpoint, stores endpoint health, and shows the raw-coordinate warning beside the custom endpoint input because user-provided endpoints can be untrusted external servers.');
...
function buildReadme() {
  ...
  return [
    ...
    '- The Android parked-car widget reads the last parked location from native storage, opens Road Sage on widget tap, launches geo navigation from its Navigate button, and uses `MapTileFetchWorker` to cache a pinned static map image per widget. That widget map refresh sends the saved parked coordinate to `staticmap.openstreetmap.de` when a refreshed preview is needed.',
    ...
    '- Native stack: Capacitor 8 Android shell plus custom Java services/plugins for activity recognition, background tracking, phone usage evidence, native IMU motion sampling, native downloads, notifications, quick settings tile, parked-car widget, WorkManager static-map cache, and SharedPreferences storage',
    ...
    '- Primary storage: IndexedDB, localStorage, Capacitor Preferences, Android SharedPreferences, native motion samples on trips, native download files, and Android widget map cache files',
    ...
    '- Optional external services: OpenStreetMap Overpass for speed limits, Open-Meteo for weather context, trusted user-configured OSRM for route snapping after explicit consent, and staticmap.openstreetmap.de for Android parked-car widget map previews. Set `VITE_OSRM_TIMEOUT_MS` to tune the build default OSRM timeout; users can override it in Settings from 5 to 30 seconds.',
    '',
    ...
    '- The generated CSP allows app code from `self`, disallows inline scripts, restricts network connections to `self`, Overpass, Open-Meteo, Nominatim, staticmap.openstreetmap.de, and `VITE_API_URL` when configured, and reports violations to `/csp-report` when served as an HTTP header.',
    ...
    '- The Android parked-car widget can fetch a static map preview for the saved parked coordinate when a widget is present and the cached preview is stale. Tapping the widget opens Road Sage; tapping Navigate uses an Android geo intent.',
