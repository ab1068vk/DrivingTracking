...
function buildDoc() {
  ...
  doc.push('- Android platform hardening: app backup is disabled and backed by backup/data-extraction exclusion XML, cleartext traffic is denied by `network_security_config.xml`, system CAs are the only trusted anchors, `FLAG_SECURE` blocks task-switcher screenshots and screen recording, Android Keystore keys request StrongBox when available, accept hardware-backed TEE when StrongBox is unavailable, and reject software-backed Keystore storage, Capacitor bridge logging is disabled, `CapacitorHttp` is disabled, WebView navigation is restricted with `server.allowNavigation: []`, release builds run R8/resource shrinking and obfuscation with `android.util.Log` calls stripped, ProGuard keeps only Capacitor reflection entry points and native JSON model members needed at runtime, and app-owned PendingIntents use `FLAG_IMMUTABLE` through `PendingIntentCompat`.');
  ...
  doc.push('- Runtime integrity: `RuntimeIntegrityCheck` detects local root, debugger, emulator, and ADB-enabled signals. `MainActivity` suspends active tracking when a compromised status is reported, and the `PlayIntegrity` bridge requests Play Integrity API tokens with per-action nonces so native calibration uploads and encrypted exports can be gated before sensitive data leaves the app. Tokens must still be decrypted and verified by a trusted backend.');
  doc.push('- User-controlled data surfaces: encrypted backup import JSON, settings import, trip/vehicle forms, route points, privacy zones, OSRM endpoint input, external context fetches, password-protected CSV/PDF/UBI export content, and Android native intent extras.');
  ...
  doc.push('- Export confidentiality: report CSV, monthly PDF, and UBI score-card exports require a 12+ character password and are saved as encrypted `.rsexport` wrappers. Full backups always export as password-protected AES-GCM `.rsbackup` files; plaintext backup export opt-out has been removed.');
  doc.push('- Secure deletion and volatile samples: Android plaintext preference cleanup, parked-widget cache deletion, native export/cache cleanup, and delete-all flows use best-effort overwrite-before-unlink through `SecureDelete`, and native motion samples are zeroed after trip handoff or when native completed trips are cleared.');
...
function buildReadme() {
  ...
  return [
    ...
    '- Native stack: Capacitor 8 Android shell plus custom Java services/plugins for activity recognition, background tracking, phone usage evidence, native IMU motion sampling, native downloads, notifications, quick settings tile, parked-car widget, WorkManager tile-preview cache, secure clipboard, Android Keystore-backed field encryption, encrypted Capacitor-style storage, biometric/device-credential gating, local runtime-integrity checks, encrypted SharedPreferences storage, and Android security hardening',
    ...
    '- Primary storage: IndexedDB and localStorage for web UI state, encrypted native key-value storage for Android UI/settings values, Android Keystore-backed encrypted trip fields for sensitive route/event/note payloads on native Android, session-only encrypted trip fields on web/test surfaces, encrypted Android SharedPreferences for native tracking, native settings, native notification state, privacy zones, and parked-location data, coarse route-risk geohash cells, native motion samples on trips, native download files, and Android widget map cache files. Android production paths migrate legacy plaintext Capacitor Preferences when possible and no longer read legacy plaintext `SharedPreferences`/Capacitor Preferences for native settings, privacy zones, tracking, notifications, or parked locations; legacy native plaintext preference files are delete-only cleanup targets.',
    ...
    '',
    '- Trips, vehicles, settings, diagnostics, and reports stay local by default. Sensitive trip fields are encrypted at rest with Android Keystore-backed native keys on Android and a non-persistent in-memory session key on web/test surfaces, and completed trips older than the configured retention window are automatically deleted unless retention is set to Never.',
    ...
    '- Android release builds disable app backup, exclude sensitive storage from backup/data extraction, deny cleartext traffic through network security config, trust only system public CAs, run R8/resource shrinking and obfuscation, strip `android.util.Log` calls with R8, mark the main window `FLAG_SECURE`, request StrongBox-backed Android Keystore keys when available while rejecting software-backed Keystore keys, keep Capacitor reflection entry points and native JSON model members in ProGuard, disable Capacitor bridge logging and `CapacitorHttp`, sync only allowlisted plugins, and use explicit immutable PendingIntent flags for app-owned intents.',
    ...
    '- `RuntimeIntegrityCheck` flags local root, debugger, emulator, and ADB-enabled signals; compromised status suspends active tracking and the `PlayIntegrity` bridge requests nonce-bound Play Integrity tokens for sensitive native actions. Token decryption and verdict verification must be performed by a trusted backend.',
