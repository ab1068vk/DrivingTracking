...
function topCalculationSnippets() {
  const snippets = [
    ...
    functionSnippet('Predictive route risk', 'src/lib/predictiveRouteRisk.js', 'estimatePredictiveRouteRisk'),
...
function buildDoc() {
  ...
  doc.push('');
  doc.push('> NOTE: Scoring thresholds and domain-significant constants now live in named registries such as `DEFAULT_THRESHOLDS`, `ECO_DEFAULTS`, `SVI_DEFAULTS`, `src/lib/appConstants.js` clock/display/storage/score-normalization constants, `ROUTE_RISK_CONSTANTS`, `RISK_CONSTANTS`, `PRE_TRIP_RISK_SIGNAL_GATES`, `HABIT_CONSTANTS`, `DAILY_FATIGUE_THRESHOLDS`, and danger-zone constants. The literal registry below remains useful for auditing labels, keys, Android IDs, and remaining inline policy values.');
  doc.push('');
  doc.push('---');
  ...
  doc.push(table(
    ...
    [
      ...
      ['Metric and confidence contract', '`src/lib/metricRegistry.js` defines display names, descriptions, sources, evidence minimums, and calibration notes for component/report metrics. `component_scores` envelopes carry value, evidence (`high`, `developing`, `low`, or `unavailable`), data sources, and sample counts; score surfaces suppress unavailable values rather than rendering zero. CSV exports add a metric-metadata row and monthly/UBI PDFs add metric-reference pages.'],
      ['Scoring calibration and provenance', '`src/lib/scoringConstants.js` owns provisional scoring/risk/UBI constants and their affected metrics. Stored score output is versioned as `SCORING_VERSION = 2.1.0`, and each newly scored trip stores `score_provenance` with its component evidence and scoring-input snapshot. Settings lists provenance/input mismatches for explicit re-score actions and marks provisional output as approximate.'],
      ...
      ['Fatigue-to-Safety deduction policy', '`FATIGUE_SAFETY_PENALTY_SCALE = 0.12` is a named provisional conversion from the normalized 0-100 fatigue proxy into raw Safety penalty points. Maximum reported fatigue therefore adds 12 raw penalty points before trip-distance normalization; this coefficient is not calibrated against collision or impairment outcome data.'],
      ...
      ['Predictive route risk evidence gate', '`estimatePredictiveRouteRisk` sorts completed trips newest-first by `startTime`/`start_time` before applying the recent-trip window and returns `status: insufficient_history` with a null risk score when the window has no observed completed-trip distance. The dashboard displays Not enough driving history rather than constructing an estimate from a default score.'],
      ...
      ['Context-aware score evidence', 'Braking-efficiency grades use urban or highway thresholds and display their driving context. Hill-driving uses named provisional GPS/altitude-derived assumptions (`HILL_ACCEL_THRESHOLD_MS2 = 2.5` and `HILL_INFRACTION_PENALTY_POINTS = 10`), returns `hill_route: false` with a null score when not applicable, and displays its measurement limitation in Trip Detail. Its absolute-count penalty should be recalibrated against labelled hill-driving evidence before a rate-based redesign. Null component evidence stays out of composite decisions and score confidence metadata supports low-data UI suppression.'],
      ['Fatigue and playback time integrity', '`calculateFatigueScore` stores a normalized 0-100 fatigue risk. Its contribution to Safety uses the provisional named `FATIGUE_SAFETY_PENALTY_SCALE`, not a validated impairment-risk model. Fatigue heatmaps use named 30-second segments and require 20 segments before display. Trip duration subtracts tracking gaps, including native Android stats, and map playback uses timestamp progress with index progress only as a missing-time fallback.'],
      ...
      ['Speed-limit fallback provenance', '`src/lib/speedLimitSource.js` supports global, Canada, United States, and United Kingdom fallback road-type profiles when OpenStreetMap supplies no posted `maxspeed` tag. Settings labels those inferred values approximate, and CSV/component source metadata reports the fallback-country provenance used for scoring.'],
      ['Predictive route normalization', '`estimatePredictiveRouteRisk` normalizes and clamps baseline, event-density, danger-zone, weather, and time inputs before applying fractional weights. `ROUTE_RISK_CONSTANTS.EVENT_DENSITY_MAX_EVENTS_PER_KM = 5` and `DANGER_ZONE_SATURATION_COUNT = 5` are named provisional saturation assumptions, not collision- or casualty-calibrated thresholds. Current `close_proximity_count` and legacy `near_miss_count` are treated as unverified brake-turn advisory counts at ordinary 1.0x event weight because stored legacy provenance is unknown; they remain excluded from trip Safety and weather score adjustment. The dashboard labels this estimate and exposes weighted signal contributions. Route-risk indexing snaps nearby GPS cells within 15 m to limit same-road fragmentation.'],
      ...
      ['UBI minimum evidence and weighting', '`computeUBIReport` returns null score, grade, tier, and category scores until 50 km is observed, uses actual distance for event rates, reduces GPS-heading-derived cornering weight to 5% while shifting weight toward braking, and uses configurable mileage assumptions that default to 10,000 km/year optimal mileage with an 8,000 km spread. Named night-driving and per-100-km event-rate deduction constants are explicitly internal approximations, not insurer-validated rates: `TIME_OF_DAY_NIGHT_MULTIPLIER = 150`, `BRAKING_PENALTY_PER_100KM = 8`, `ACCEL_PENALTY_PER_100KM = 8`, `CORNERING_PENALTY_PER_100KM = 6`, and `SPEED_PENALTY_PER_100KM = 10`.'],
      ['Commute and coaching policy', '`COMMUTE_MATCH_RADIUS_M` documents the 225 m commute route-match radius shown in Settings. Weekly coaching uses one focus metric and requires a score delta greater than 3 points; score tips require at least 2 km and confidence of at least 0.5.'],
      ...
      ['Development verification fixtures', 'Development Diagnostics can seed and remove twelve local synthetic completed trips. Human-verified scoring golden fixtures lock the `2.1.0` score contract, while a shared Android/JavaScript parity fixture exercises native stats, local-time night classification, and the conservative GPS noise-floor calculation.'],
  ...
  doc.push('');
  doc.push('Named thresholds and policies are centralized around `SCORING_CONSTANTS`, `DEFAULT_THRESHOLDS`, `ECO_DEFAULTS`, `SVI_DEFAULTS`, `PENALTY_SCALE_FACTOR`, `FATIGUE_SAFETY_PENALTY_SCALE`, `src/lib/appConstants.js`, `PERSONAL_BASELINE_MIN_TRIPS`, `FATIGUE_HEATMAP_SEGMENT_SECONDS`, `COMMUTE_MATCH_RADIUS_M`, `MIN_UBI_REPORT_DISTANCE_KM`, `TIME_OF_DAY_NIGHT_MULTIPLIER`, UBI per-100-km rate deduction constants, predictive and segment-index `ROUTE_RISK_CONSTANTS`, pre-trip-risk, habit-profile, daily-fatigue, and danger-zone constants. `METRIC_REGISTRY` supplies exported metric descriptions, evidence minimums, sources, and calibration notes. The app also contains intentional literals for route labels, feature flags, UI labels, named Android notification IDs, and tests; these are grouped by file so reviewers can see why each value exists.');
  doc.push('');
  ...
  doc.push('');
  doc.push('Core persisted models are plain JSON trip, vehicle, settings, backup, diagnostic, route-risk-index, privacy-zone, and Android native event records. Completed trips now include canonical `component_scores` evidence envelopes and `score_provenance` for scoring version/input auditing. There is no ORM schema in this repo; IndexedDB schema creation lives in `src/lib/localTripRepository.js`, local/mobile storage helpers live in `src/lib/mobileStorage.js` and `src/lib/trackingStore.js`, and Android SharedPreferences records live in `DriveSenseNativeTripStore.java` and `DriveSenseAutoTrackingService.java`.');
  doc.push('');
...
function buildReadme() {
  ...
  return [
    ...
    '- Trip scoring for safety, smoothness, eco driving, confirmed Android Usage Access phone-use distraction, speed compliance, road-type segments, brake onset smoothness, cornering, braking efficiency, stop-start patterns, fatigue, heading drift Beta, slippery-condition, estimated brake-turn manoeuvre alerts, route-risk proxies, and diagnostic-only GPS phone/overtake pattern counts.',
    ...
    '- Reports with CSV export carrying metric provenance metadata, monthly and UBI PDF metric-reference pages, UBI score-card PDF export gated until 50 km of evidence, confidence-aware rolling baseline comparison, carbon impact, configurable-currency fuel cost, and vehicle-backed CO2 savings estimates.',
    ...
    '| `stop_start_pattern_*` | Repeated cruise then deceleration/speed-drop cycles in GPS speed data. | Hidden until 5 km of highway evidence exists and contributes 5% of Safety; it cannot measure following distance. |',
    '| `close_proximity` manoeuvre alert | At least 1.5 s of coincident braking and heading change at 30+ km/h; defaults are 4.0 m/s2 and 25 deg/s. | Always low confidence and advisory-only for Safety and weather score adjustment; predictive route-pattern review uses ordinary 1.0x event weight. It does not establish object proximity or a near miss. |',
    ...
    '- Fatigue contribution to Safety now uses named `FATIGUE_SAFETY_PENALTY_SCALE = 0.12`: maximum normalized fatigue adds 12 raw Safety penalty points before trip-distance normalization. This provisional coefficient is not calibrated to collision or impairment outcomes.',
    ...
    '- `stop_start_pattern_score` replaces following-distance claims for new scored trips. It is low confidence, hidden below 5 km of highway evidence, contributes 5% of Safety only when present, and is the only Defensive proxy in that slot because GPS speed cannot measure vehicle gap.',
    ...
    '- GPS heading-deviation detection emits low-confidence `heading_deviation` / Heading Event (Beta) records only when advanced safety detection is enabled. It requires a straight approach above 50 km/h, suppresses common context windows, and no longer affects Safety scoring.',
    '- Estimated brake-turn manoeuvre alerts require at least 1.5 seconds of concurrent braking and heading change at 30+ km/h with default thresholds of 4.0 m/s2 and 25 deg/s. They remain low-confidence advisory signals and are excluded from Safety and weather score adjustment; predictive route-pattern review uses ordinary 1.0x event weight.',
    ...
    '- Monthly PDF exports include the GPS-only proxy limitation alongside Safety and Smoothness results; GPS phone and overtake patterns are described as diagnostics only, and heading events plus Heading Drift Beta are shown only when advanced detection was enabled for the rescored trip. The central metric registry now supplies evidence/source/calibration metadata to CSV exports and metric-reference pages in monthly and UBI PDFs.',
    ...
    '- Braking-efficiency grades are contextual: urban and highway driving use separate thresholds, and the displayed grade identifies its context. Hill control now names its provisional GPS/altitude-derived threshold and penalty assumptions (`2.5 m/s2` and `10` points per inferred infraction), stores flat or altitude-insufficient routes as not applicable, and displays its GPS-only limitation in Trip Detail.',
    '- Score confidence is evidence-aware rather than distance-only. Safety, Smoothness, Eco, and Distraction now expose `high`, `developing`, `low`, or `unavailable` confidence based on their contributing signals; unavailable phone-use or intersection evidence prevents a long trip from appearing fully evidenced. Trip Detail displays component confidence, and coaching remains suppressed when overall evidence is low.',
    ...
    '- Predictive route risk requires observed completed-trip distance; without it, the dashboard shows Not enough driving history and no risk number. Once history exists, it sorts completed trips newest-first, excludes tiny-trip event-density distortion, normalizes all weighted components to 0-100, and clamps weather/baseline input before scoring. Its named saturation assumptions remain provisional: five eligible route events per km or five nearby learned danger zones each saturate their respective signal without a collision-outcome calibration claim. Current brake-turn alerts and ambiguous legacy `near_miss` counts use ordinary 1.0x event-density weight because their stored provenance is not distinguishable. The dashboard labels the value as estimated and exposes weighted signal contributions. Route-risk index cells are coarser and merge nearby segment midpoints within 15 m to reduce GPS fragmentation.',
    '- UBI reports require at least 50 km before generating a score, use actual distance in per-100-km rates, score time-of-day exposure by night driving minutes, reduce noisy GPS-derived cornering to a 5% weight, and use configurable mileage assumptions that default to an optimal 10,000 km/year with an 8,000 km spread. The named night-exposure and category-rate penalty constants remain internal, uncalibrated approximations rather than insurer-validated rates; the score-card UI now presents that limitation alongside any displayed score.',
    ...
    '- Local trip storage uses IndexedDB when available, with a migration runner and localStorage fallback. Legacy/schema-upgrade refreshes populate current component evidence and score provenance; Settings identifies current scoring-input/version mismatches and lets the user deliberately queue affected completed trips for re-score.',
    ...
    '- Open road context is explicit and privacy-aware. OpenStreetMap speed limits and Open-Meteo weather are manual by default unless automatic context fetch is enabled. When a posted map speed limit is absent, Settings can choose Global, Canada, United States, or United Kingdom approximate road-type defaults, with the chosen fallback provenance preserved in reports. OSRM route snapping is opt-in, disabled without a configured endpoint, the public demo requires confirmation, and the custom endpoint field warns that raw sampled GPS coordinate pairs are sent to the configured OSRM server.',
    '- Trip Detail and Map no longer silently hide additional route-risk stretches or risk hotspots: initial lists remain compact, and show-all controls report hidden counts. Heading drift Beta color and fatigue critical markers now follow actual levels and exported thresholds, and compliance bars use the canonical score color tiers.',
    ...
    '- Commute matching uses the named `COMMUTE_MATCH_RADIUS_M` 225 m threshold exposed in Advanced settings, while weekly coaching avoids duplicate metric advice and only comments on score changes larger than 3 points.',
