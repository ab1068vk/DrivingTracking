...
function buildDoc() {
  ...
  doc.push(table(
    ...
    [
      ...
      ['Stop-start pattern proxy', '`detectTailgateCycles` emits low-confidence `stop_start_pattern` events from GPS speed only. `stop_start_pattern_score` is withheld until at least 5 km of highway evidence is present and contributes 5% of Safety; the app explicitly states it cannot measure lead-vehicle distance.'],
      ...
      ['Heading event Beta', '`detectLaneChanges` now emits low-confidence `heading_deviation` events labelled Heading Event (Beta) only within advanced-safety detection. These events are shown for review and no longer deduct from Safety scoring.'],
...
function buildReadme() {
  ...
  return [
    ...
    '| --- | --- | --- |',
    '| `brake_onset_smoothness_*` | Ramp duration and peak GPS-derived deceleration during detected harsh braking. | Reports only after five sequences, is always low confidence, and contributes 10% of Smoothness. The UI disclaimer states it is not neurological reaction time. |',
    '| `stop_start_pattern_*` | Repeated cruise then deceleration/speed-drop cycles in GPS speed data. | Reports only after 5 km of highway evidence and contributes 5% of Safety; it cannot measure following distance. |',
    ...
    '- `stop_start_pattern_score` replaces following-distance claims for new scored trips. It is low confidence, withheld below 5 km of highway evidence, and reduced to 5% of Safety because GPS speed cannot measure vehicle gap.',
    '- `brake_onset_smoothness_*` replaces public reaction-time output in the UI and CSV. It uses peak deceleration divided by ramp duration, requires five detected braking sequences, is always low confidence, and displays a disclaimer that it is not neurological reaction time.',
