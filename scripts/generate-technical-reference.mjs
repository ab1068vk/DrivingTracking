...
function buildDoc() {
  ...
  doc.push(table(
    ...
    [
      ...
      ['UBI minimum evidence and weighting', '`computeUBIReport` returns insufficient data below 50 km, uses actual distance for event rates, reduces GPS-heading-derived cornering weight to 5% while shifting weight toward braking, and uses configurable mileage assumptions that default to 10,000 km/year optimal mileage with an 8,000 km spread.'],
...
function buildReadme() {
  ...
  return [
    ...
    '| `close_proximity` manoeuvre alert | At least 1.5 s of coincident braking and heading change at 30+ km/h; defaults are 4.0 m/s2 and 25 deg/s. | Always low confidence, carries a conservative Safety penalty and close-proximity Safety component, and has 1.5x route/weather-risk influence. It does not establish object proximity or a near miss. |',
    ...
    '- Estimated brake-turn manoeuvre alerts require at least 1.5 seconds of concurrent braking and heading change at 30+ km/h with default thresholds of 4.0 m/s2 and 25 deg/s. They carry a conservative Safety impact, remain low confidence, and route/weather risk weighting is reduced from 2x to 1.5x.',
    ...
    '- Monthly PDF exports include the GPS-only proxy limitation alongside Safety and Smoothness results; GPS phone and overtake patterns are described as diagnostics only, and heading events plus Heading Drift Beta are shown only when advanced detection was enabled for the rescored trip.',
    ...
    '- Local trip storage uses IndexedDB when available, with a migration runner and localStorage fallback. Trip schema versioning triggers rescoring for completed trips when scoring, confidence, phone-use source gating, corrected duration, map, or privacy behavior changes.',
