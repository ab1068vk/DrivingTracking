...
function purposeFor(file) {
  ...
  if (file.startsWith('src/engine/')) return 'Focused domain engine module for scoring, detection, calibration, routing, export, or shared math.';
  if (file.startsWith('src/settings/')) return 'Modular Settings screen section, navigator, or shared Settings UI component.';
  if (file.startsWith('src/features/')) return 'Feature-scoped React component or hook.';
...
function dependencyTable() {
  return table(
    ...
    packageFacts().map((dep) => [
      ...
      dep.name === 'html2canvas' ? 'Pinned exactly with a package override while SECURITY-HOLD tracks upgrade/replacement; monitored by npm audit and Dependabot.'
        : dep.name === 'react-markdown' ? 'Monitored by npm audit and Dependabot; raw-HTML plugin `rehype-raw` is banned by ESLint.'
          : 'Monitored by CI npm audit gates and weekly Dependabot update PRs; lockfile pins the installed version for reproducible audit results.',
    ]),
  );
}
...
function envTable() {
  ...
  const envDescriptions = {
    ...
    VITE_DEFAULT_MAP_LAT: 'Optional last fallback latitude for the TripPlayback review surface when no trip, settings, privacy-zone, parked, or device context exists.',
    VITE_DEFAULT_MAP_LNG: 'Optional last fallback longitude for the TripPlayback review surface when no trip, settings, privacy-zone, parked, or device context exists.',
...
function topCalculationSnippets() {
  const snippets = [
    functionSnippet('Score evidence envelopes and export-facing component contract', 'src/engine/calibration/baseline.js', 'createComponentScore'),
    functionSnippet('Score provenance snapshot and version checks', 'src/engine/calibration/baseline.js', 'buildScoreProvenance'),
    functionSnippet('Trip scoring weights, confidence, diagnostic gates, and final score', 'src/engine/scoring/pipeline.js', 'calculateTripScores'),
    functionSnippet('Trip stats and gap-corrected duration', 'src/engine/scoring/pipeline.js', 'calculateTripStats'),
    functionSnippet('Eco score, cruise band, idle penalty', 'src/engine/scoring/ecoScore.js', 'calculateEcoDrivingScore'),
    functionSnippet('Driving-event detection pipeline', 'src/engine/detection/harshBraking.js', 'detectDrivingEvents'),
    ...
    functionSnippet('Route risk geohash index and privacy-preserving lookup', 'src/lib/routeRiskIndex.js', 'buildRouteRiskIndex'),
  ...
  const androidFile = 'android/app/src/main/java/com/roadsage/app/RoadSageAutoTrackingService.java';
...
function buildDoc() {
  ...
  doc.push(table(
    ...
    [
      ...
      ['OSRM consent and health checks', 'OSRM route snapping is disabled until the user saves a private/trusted endpoint, explicitly consents to sending sampled GPS coordinates, and the endpoint passes a 200 OPTIONS verification with an exposed OSRM-specific response header. The app records endpoint health, prominently displays the configured domain, re-verifies at launch, and disables snapping if health fails. The public demo endpoint is shown as an example but is rejected for saved settings; `VITE_DEFAULT_OSRM_URL` can prefill trusted deployments and `VITE_OSRM_TIMEOUT_MS` sets the build default timeout before the user overrides it.'],
      ...
      ['Tracking health and permission monitoring', '`usePermissionMonitor` checks foreground/background location, activity recognition, notifications, and Android battery optimization on load, focus, visibility change, and a 60-second cadence. Dashboard displays `PermissionWarningBanner` and `TrackingHealthChip` when background auto tracking is degraded or cannot be verified.'],
      ...
      ['Backup confidentiality and integrity', '`src/lib/backupEncryption.js` encrypts password-protected backups with PBKDF2-HMAC-SHA256 and AES-256-GCM. Plaintext opt-out backups are HMAC-sealed with an install-bound key, imports verify the seal before merge, encrypted imports require the password, and Settings defaults export to encrypted backups with a 12-character password requirement, confirmation field, strength meter, unencrypted opt-out warning, wrong-password messaging, and corrupted encrypted-backup error copy.'],
  ...
  doc.push('- Android platform hardening: app backup is disabled and backed by backup/data-extraction exclusion XML, cleartext traffic is denied by `network_security_config.xml`, system CAs are the only trusted anchors, `FLAG_SECURE` blocks task-switcher screenshots and screen recording, Capacitor bridge logging is disabled, `CapacitorHttp` is disabled, WebView navigation is restricted with `server.allowNavigation: []`, release builds run R8/resource shrinking and obfuscation with `android.util.Log` calls stripped, ProGuard keeps only Capacitor reflection entry points and native JSON model members needed at runtime, and app-owned PendingIntents use `FLAG_IMMUTABLE` through `PendingIntentCompat`.');
  doc.push('- Capacitor plugin surface: `capacitor.config.ts` uses `includePlugins` allowlists for the Capacitor and background-geolocation plugins Road Sage actually syncs, while custom first-party bridge names such as `SecureClipboard` and `DriveSenseActivityRecognition` are explicitly documented in plugin config. Native `MainActivity` registers only the Road Sage Java plugins it owns.');
  ...
  doc.push('- Biometric session lock: `src/lib/biometricLock.js` centralizes the unlocked session state. Privacy & Data settings store `lock_timeout_minutes` with a 5-minute default and a 0-30 minute validated range; `src/App.jsx` also locks immediately on document visibility loss or Capacitor inactive app state, so screen-off/background transitions clear the unlocked session even when the timeout is set to Never.');
  ...
  doc.push('- Calibration timestamp privacy: shared calibration labels use summary-only payloads and protect the trip-start timestamp with Laplace noise calibrated to epsilon=1.0 and one-hour sensitivity before hour rounding, limiting correlation between upload logs, model version, trip features, and exact trip time.');
  doc.push('- GPS coordinate minimization: route points, parked-location coordinates, and last-map-center coordinates are rounded to 5 decimal places before local or native storage; altitude is rounded to whole meters.');
  doc.push('- Android certificate pinning: `network_security_config.xml` pins the built-in OpenStreetMap/Nominatim, Open-Meteo, and Overpass hosts with at least two SHA-256 SPKI pins per host. Security CI runs `scripts/check_pin_expiry.py android/app/src/main/res/xml/network_security_config.xml 60` so expired, soon-expiring, missing, placeholder, or malformed pins fail before release.');
  doc.push('- OSRM endpoint validation: saved route-snapping endpoints must pass a 200 OPTIONS check with an exposed `X-OSRM-*` or OSRM-valued response header; launch re-verification disables snapping and notifies the user if the endpoint stops responding.');
  ...
  doc.push('- Leaflet maps: `TripMap.jsx` and `TripPlayback.jsx` statically import Leaflet and `leaflet/dist/leaflet.css` from the bundled npm dependency. They do not dynamically inject Leaflet `<script>` tags or load Leaflet from a CDN, removing a third-party script execution path over decrypted trip state.');
  ...
  doc.push('- Notification privacy: Android notification channels set explicit lock-screen visibility. Active tracking foreground notifications use secret visibility and generic text; post-trip, coaching, fatigue, achievement, and export notifications use private visibility so bodies are hidden on the lock screen.');
  doc.push('- Clipboard privacy: app-initiated clipboard writes go through `SecureClipboard`, mark native Android clips as sensitive where supported, and auto-clear the clipboard after 60 seconds if the copied value is still present.');
  doc.push('- Dependency and pin monitoring: security CI runs production `npm audit --audit-level=high --omit=dev`, all-dependency critical audit, Android certificate-pin expiry checks, lint, tests, Android instrumentation, build checks, and production-bundle debug-route scans. Dependabot is configured for npm and Gradle update PRs, with security-sensitive package grouping.');
  doc.push('- Backup restore: schema versions 1 through 6 are migrated through `BACKUP_MIGRATIONS` before merge; retired `lane_change` events are relabelled as diagnostic `heading_deviation_legacy` records; encrypted backups require a password and use AES-GCM authentication, plaintext opt-out backups are HMAC-sealed and verified before import, untrusted records are whitelisted and field-limited, newer-backup errors tell users to update Road Sage, and any note truncation reports the affected trip count and requires explicit user confirmation before completion.');
...
function buildReadme() {
  ...
  return [
    ...
    '- Full backup export/import for trips, GPS route points, events, vehicles, settings, privacy-zone metadata, saved filters, reviewed event feedback, score explanations, lane-changing evidence, fallback speed-limit provenance, and legacy heading-event migration. Exports default to password-protected AES-GCM `.rsbackup` files; plaintext opt-out backups are HMAC-sealed, and imports require confirmation before truncating notes. Stealth trips are intentionally excluded from persistence and backup history.',
    '- Trip history and dashboard identify completed trips scored with older settings/provenance and offer deliberate re-score flows so historical cards, baselines, and reports can be refreshed after scoring input changes.',
    ...
    '- Trip Detail now uses `TripEventList` to separate scored driving events from diagnostic and Beta data, labels diagnostic rows consistently, and passes completed-trip count into score confidence displays.',
    ...
    '- Local trip storage uses IndexedDB when available, with a migration runner and localStorage fallback. Sensitive trip fields are encrypted at rest, backups omit encrypted repository payloads in favor of sanitized export records, and completed trips older than the configurable retention period are pruned automatically; the default retention is 24 months. `VITE_DB_NAME` can rename the local database with a copy/count-verify/delete migration. `src/lib/schema/tripSchema.js` is the canonical v23 trip-record reference for required current fields. Legacy/schema-upgrade refreshes populate current component evidence, score provenance, and score explanations; Settings and dashboard identify current scoring-input/version mismatches, auto-re-score recent trips when more than 20% of the 28-day window is stale, and Trip History lets the user deliberately queue affected completed trips for re-score.',
    ...
    '- Android tracking updates include immediate native notification state, lock-screen visibility controls with generic secret active-tracking notifications and private post-trip/coaching bodies, configurable biometric session auto-lock with immediate lock on background/screen-off, quick settings tile sync, clearer off/paused handling, named notification identifiers, device-local fixed-hour night classification aligned to the shared 22:00-04:59 window, deduplicated trip/safety notifications, battery optimization guidance, phone usage access support, native IMU motion samples capped at 5,000 per trip, parked-car widget refresh hooks, encrypted native trip/privacy storage, secure WebView/window/backup/network defaults, immutable app-owned PendingIntents, release log stripping, and native diagnostics surfaced in the app. Android Gradle setup now removes obsolete AGP flags and reapplies clean AGP 9-compatible plugin DSL patches after install or sync.',
    ...
    '- Native stack: Capacitor 8 Android shell plus custom Java services/plugins for activity recognition, background tracking, phone usage evidence, native IMU motion sampling, native downloads, notifications, quick settings tile, parked-car widget, WorkManager tile-preview cache, secure clipboard, encrypted SharedPreferences storage, and Android security hardening',
    ...
    '- Primary storage: IndexedDB, localStorage, Capacitor Preferences for web/non-native UI state, encrypted IndexedDB trip fields for sensitive route/event/note payloads, encrypted Android SharedPreferences for native tracking, native settings, native notification state, privacy zones, and parked-location data, coarse route-risk geohash cells, native motion samples on trips, native download files, and Android widget map cache files. Android production paths no longer read legacy plaintext `SharedPreferences`/Capacitor Preferences for native settings, privacy zones, tracking, notifications, or parked locations; legacy native plaintext preference files are delete-only cleanup targets.',
    ...
    '- Optional external services: OpenStreetMap Overpass for speed limits, Open-Meteo for weather context, Nominatim reverse geocoding outside privacy zones, trusted user-configured OSRM for route snapping after explicit consent, and pinned OpenStreetMap tile hosts for non-private Android parked-car widget map previews. Set `VITE_OSRM_TIMEOUT_MS` to tune the build default OSRM timeout; users can override it in Settings from 5 to 30 seconds.',
    '',
    '## Privacy And Security Defaults',
    '',
    '- Trips, vehicles, settings, diagnostics, and reports stay local by default. Sensitive trip fields are encrypted at rest in IndexedDB, and completed trips older than the configured retention window are automatically deleted unless retention is set to Never.',
    '- No ads are implemented. Calibration-label sharing is opt-in only; without that setting, survey feedback stays local. Shared calibration payloads are summary-only and protect the trip-start timestamp with Laplace noise before hour rounding; raw GPS, exact addresses, route polylines, personal identifiers, and trip notes are never included.',
    '- The generated CSP allows app code from `self`, disallows inline scripts, restricts network connections to `self`, Overpass, Open-Meteo, Nominatim, and `VITE_API_URL` when configured, and reports violations to `/csp-report` when served as an HTTP header. Android also injects CSP and security headers into Capacitor WebView local responses.',
    '- Android release builds disable app backup, exclude sensitive storage from backup/data extraction, deny cleartext traffic through network security config, trust only system public CAs, run R8/resource shrinking and obfuscation, strip `android.util.Log` calls with R8, mark the main window `FLAG_SECURE`, keep Capacitor reflection entry points and native JSON model members in ProGuard, disable Capacitor bridge logging and `CapacitorHttp`, sync only allowlisted plugins, and use explicit immutable PendingIntent flags for app-owned intents.',
    ...
    '- Biometric unlock state uses a configurable auto-lock timeout stored as `lock_timeout_minutes` with a 5-minute default; the app also locks immediately when it backgrounds, loses visibility, or the screen turns off.',
    ...
    '- OSRM route snapping is disabled until the user saves a trusted endpoint, consents to raw sampled GPS coordinate sharing, and the endpoint passes a 200 OPTIONS check with an exposed `X-OSRM-*` response header. The endpoint domain is shown prominently in Settings, health is re-verified at app launch, and snapping is disabled if verification fails.',
    ...
    '- Imported backups and settings are treated as untrusted input, migrated from supported legacy schemas, sanitized before merge, and require confirmation before any note-truncating import completes. Password backups use AES-GCM encryption with a Settings strength meter and confirmation field; plaintext opt-out backups show an unencrypted warning, are sealed with an install-bound HMAC, and are rejected if tampered.',
