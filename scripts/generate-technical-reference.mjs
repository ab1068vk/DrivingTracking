...
function buildDoc() {
  ...
  doc.push(table(
    ...
    [
      ...
      ['Following distance reliability', '`detectTailgateCycles` observes close-following deceleration patterns from urban speeds upward and breaks patterns at privacy-masked route points. `calculateTripScores` normalizes speed-weighted tailgate penalties over total route distance rather than highway kilometres; routes under 0.5 km report a null following score, and scored routes use the full 0-100 range.'],
...
function buildReadme() {
  ...
  return [
    ...
    '- Shared application policy now lives in `src/lib/appConstants.js`: fallback night and rush-hour boundaries are consistent across habit, predictive-route, pre-trip, trip-tagging, trip-engine fallback, settings defaults, and Android fixed-hour classification. Legacy sunset-mode defaults migrate from 06:00 to the shared 05:00 end; deliberately custom night hours remain configurable. Saved UI preference keys and initial display limits are named in one place.',
    ...
    '- Following-distance scoring now recognizes close-following deceleration patterns on city and highway routes, excludes privacy-masked evidence, weights higher-speed patterns more strongly, and normalizes penalties by total route distance. Trips under 0.5 km expose no following score; scored trips can reach 0 without the former 20-point floor.',
    ...
    '- Eco driving scoring now resolves missing or malformed tuning through named `ECO_DEFAULTS`, clamps impossible parked-idle ratios, reports invalid zero-multiplier configurations as unavailable rather than a fixed score, and blends remaining eco evidence into the trip score.',
    ...
    '- CO2 savings are backfilled for legacy completed trips. Missing `co2_saved_kg` now triggers trip rescoring in local storage, and carbon reports estimate savings for older records that have distance and eco data but predate the field.',
