// Synchronous SHA-256 / HMAC-SHA-256.
//
// crypto.subtle only offers the async form, and the privacy-zone cell lookup
// runs inside isPointInPrivacyZone, which is synchronous and called once per
// route point on both the masking and the outbound-filter paths. Making that
// path async would mean rewriting every caller, so the primitive is provided
// here instead. Android uses javax.crypto.Mac for the same computation.
...
export function sha256(message) {
  ...
  const padded = new Uint8Array(((((length + 8) / BLOCK_BYTES) | 0) + 1) * BLOCK_BYTES);
  padded.set(message);
  padded[length] = 0x80;
 ...
 * @param {Uint8Array} message
 * @returns {Uint8Array} the 32-byte tag
 */
...
export function toHex(bytes, byteCount = bytes.length) {
  let hex = '';
