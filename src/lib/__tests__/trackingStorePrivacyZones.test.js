...
vi.mock('@capacitor/core', () => ({
  Capacitor: {
    ...
    isNativePlatform: vi.fn(() => capacitorState.native),
  },
...
describe('trackingStore privacy zones', () => {
  ...
  it('does not fall back to Capacitor Preferences on Android when the native plugin is unavailable', async () => {
    capacitorState.platform = 'android';
    capacitorState.native = true;
    preferenceState.values.set(PRIVACY_ZONES_KEY, JSON.stringify([{ name: 'Home', lat: 43.65, lng: -79.38, radius: 200 }]));

    expect(await getPrivacyZones()).toEqual([]);
    await savePrivacyZones([{ name: 'Work', lat: 43.66, lng: -79.39, radius: 150 }]);

    expect(Preferences.get).not.toHaveBeenCalled();
    expect(preferenceState.values.get(PRIVACY_ZONES_KEY)).toContain('Home');
  });
