...
describe('release blocker regressions', () => {
  ...
  it('keeps manual tracking background limits visible and persistent', () => {
    ...
    expect(dashboardSource).toContain('speakTripStartConfirmationOnce');
    expect(dashboardSource).toContain('candidate_tracking_ready_voice_alert');
    ...
    expect(nativeServiceSource).toContain('!isSettingEnabled("trip_start_voice_alert_enabled", true)');
    expect(nativeServiceSource).not.toContain('trackingReadyAlertSpoken || !isSettingEnabled("voice_alerts_enabled", true)');
    ...
    expect(nativeServiceSource).not.toContain('getSettingDouble("inferred_voice_margin_kmh"');
    ...
    expect(nativeServiceSource).toContain('DANGER_ZONES_KEY');
    expect(nativeServiceSource).toContain('evaluateRepeatedEventAreaAlert');
    expect(nativeServiceSource).toContain('nativeActiveCoachBriefMessage');
    expect(nativeServiceSource).toContain('isNativeVoiceAlertTypeEnabled("speeding")');
    expect(nativeServiceSource).toContain('isSettingEnabled("voice_driving_event_alerts_enabled", true)');
    expect(nativeServiceSource).toContain('isSettingEnabled("voice_attention_incident_alerts_enabled", true)');
    expect(nativeServiceSource).toContain('isSettingEnabled("voice_coaching_reminder_alerts_enabled", true)');
    ...
    expect(settingsSource).toContain('Trip start voice confirmation');
    expect(settingsSource).toContain('trip_start_voice_alert_enabled');
    expect(settingsSource).toContain('Live voice alerts — master switch');
    expect(settingsSource).toContain('Choose which recurring alerts speak');
    expect(settingsSource).toContain('These switches control voice only');
    expect(settingsSource).toContain('VOICE_ALERT_CONTROL_GROUPS.map');
    expect(settingsSource).toContain('Test voice accepted by the device speech output.');
