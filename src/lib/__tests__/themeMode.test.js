...
import { applyThemeMode, migrateDefaultSettings } from '@/lib/trackingStore';
...
describe('theme mode', () => {
  ...
  it('falls back to the system theme when a saved theme is no longer supported', () => {
    applyThemeMode('retired_theme');
    expect(documentElement.dataset.themeMode).toBe('system');
    expect(['light', 'dark']).toContain(documentElement.dataset.resolvedTheme);

    const migrated = migrateDefaultSettings({ dark_mode: 'retired_theme' });
    expect(migrated.settings.dark_mode).toBe('system');
