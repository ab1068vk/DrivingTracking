...
export function tripsToCSV(trips) {
  const headers = [
    ...
    'Smooth Braking', 'Engine Stress', 'Tire Wear Units', 'GPS Attention Signal', 'Phone Proxy (Diagnostic)', 'Parking Score',
    ...
    'Overtake Quality Score (Development Diagnostic)', 'Overtake Pattern Count (Development Diagnostic)', 'Unsafe Re-entry Count (Development Diagnostic)',
    'Weather Context', 'Safety Condition Bonus',
    'Road Type', 'Harsh Brakes', 'Rapid Accels', 'Sharp Turns', 'Speeding Events',
    'Heading Deviation Events (Diagnostic)', 'Heading Events (Legacy)', 'Stop-Start Patterns', 'Erratic Speed Events', 'Brake-Turn Manoeuvre Alerts', 'Overtake Patterns (Development Diagnostic)', 'Night Driving',
  ...
  const rows = trips.map((rawTrip) => {
    ...
    scoreCsvValue('Overtake Quality Score (Development Diagnostic)', t.overtake_quality_score),
    t.overtake_count ?? '',
