...
export function PremiumHistorySearch({
  ...
  const dateOptions = quickFilters.filter((option) => DATE_FILTER_IDS.has(option.id));
  const tripOptions = quickFilters.filter((option) => TRIP_FILTER_IDS.has(option.id));
  const dateValue = DATE_FILTER_IDS.has(filterBy) ? filterBy : 'all';
    ...
    <section className="premium-history-filter" aria-label="Premium trip history search">
      ...
      <div className="premium-history-control-grid" role="group" aria-label="Premium trip history filters">
        <label className="premium-history-control">
          ...
          <select aria-label="Filter trips by date" value={dateValue} onChange={(event) => onFilterChange(event.target.value)}>
            <option value="all">Any date</option>
            {dateOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}
          </select>
        </label>
