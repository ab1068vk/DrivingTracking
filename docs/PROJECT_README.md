...
- Eco driving scoring now resolves missing or malformed tuning through named `ECO_DEFAULTS`, clamps impossible parked-idle ratios, penalizes sustained parked idle instead of unavoidable traffic-stop idle, reports invalid zero-multiplier configurations as unavailable rather than a fixed score, and blocks Settings changes that would disable both eco multipliers. Eco evidence is still stored and displayed as `score_eco`, but it is not blended into the headline Overall trip score.
