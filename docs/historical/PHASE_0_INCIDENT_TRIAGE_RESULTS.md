> **Historical record.** This document describes a one-time verification, measurement or
> branch-specific investigation. It is retained for context and is **not** current
> architecture or release documentation. For current behaviour see
> [docs/README.md](../README.md). Enduring content has been folded into the maintained
...
- These numbers are a historical baseline from one physical device and data set.
- Route logging and repository maintenance are now deferred through idle callbacks.
- Trip History and Saved road speeds now virtualize long lists.
- Saved road speeds still schedules a full-trip geometry read of up to 500 trips when the map model is needed, so this remains the main path to retest.
- A fresh device run is needed before calling the lag incident resolved.
...
Prioritize retesting full-trip repository reads/maintenance and the Saved road speeds map-model load on the current build. Do not prioritize Leaflet rendering, route logging, result limiting alone, or the measured resume tasks based only on this historical run.
