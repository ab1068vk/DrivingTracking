...
Road Sage is local-first. Trip history, GPS routes, scores, events, vehicles, settings, privacy zones, calibration labels, active-trip recovery, and parked-location records stay on this device unless you choose to export them, import/share a backup, enable a configured backend, upload calibration labels, or use an external road, map, weather, or route-matching service.

## What Stays Local

- Trips, route points, driving events, score evidence, vehicles, settings, privacy zones, calibration labels, active-trip recovery, and parked-location records are stored locally by default.
...
Privacy zones clip visible routes to the zone edge, hide events inside the zone guard, suppress private parked-location storage, and remove privacy-zone center coordinates from backups.

Privacy zones do not undo data already shared, exported, downloaded, screenshotted, backed up, or stored outside Road Sage.

## Exports And Backups

- Exports and backups are encrypted with the password you choose.
...
- Downloaded files and files shared outside the app are outside Road Sage deletion controls.
