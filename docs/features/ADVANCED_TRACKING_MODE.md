...
- Capture fidelity is **not** governed by `experience_mode`. `capture_fidelity` is a separate setting ([../architecture/CAPTURE_FIDELITY.md](../architecture/CAPTURE_FIDELITY.md)) precisely so that switching presentation mode never changes what lands on disk — trips recorded in either mode stay comparable.
